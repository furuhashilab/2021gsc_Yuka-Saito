...
【　STEP1: カラー挿入 】  
唯一このステップだけは、どのキャラクターのデザインであっても共通して最初に行うべきだと思ったものである。まず、材料準備で用意したメインカラーをbaseカラーとして設定しサブカラーである２色をWater,roadに設定。さらにアクセントカラーをあまった項目に仮で割り振っていく。(schoolやHospitalがおすすめ)このように、キャラクターデザイン上で多く使用されていた色を使用面積の広い順に地図デザインに落とし込んでいく。このカラー割り振りをする際には地図上の場所は未だどこでもいいが、カラーの使用量が多いキャラクターなどの場合は画面上にbaseやroad以外の森林や川、学校や病院などあらゆるカラーを使用できるように多くのカテゴリが密集する都市が
